...
Alter Table Ocurrencias add constraint Fk_Ocur_Sed foreign key(sede) references sedes(id) ON DELETE CASCADE;
Alter Table Ocurrencias add constraint Fk_Ocur_Hob foreign key(hobby) references hobbies(numero);
Alter Table Telefonos add constraint Fk_Tel_Emp foreign key(tdoc,numdoc) references Empleados(tdoc,numdoc) ON DELETE CASCADE;
Alter Table Ofrecimientos add constraint Fk_Ofr_Sed foreign key(sede) references sedes(id) ON DELETE CASCADE;
Alter Table Ofrecimientos add constraint Fk_Ofr_Eve foreign key(evento) references eventos (id) ON DELETE CASCADE;
Alter Table Empleados add constraint Fk_Emp_Sed foreign key(sede) references sedes(id) ON DELETE SET NULL;
